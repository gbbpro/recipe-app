...
export default function PlannerPage() {
  ...
  const [shoppingList, setShoppingList] = useState<
    { recipeId: number; name: string; ingredients: string[] }[]
  >([]);
  ...
  const generateShoppingList = async () => {
    ...
    const stripped = allIngredients.map((item) =>
      item
        ...
        .replace(
          /^\s*(cups?|tbsp?|tablespoons?|teaspoons?|tsp?|oz|ounces?|lbs?|pounds?|g|kg|ml|l|pinch|handful|dash|cloves?|slices?|cans?|packages?|pkg|bunch|head|large|medium|small|inch|cm)\s+/i,
          "",
        ) // strip units
                  ...
                  {DAYS.map((_, dayIndex) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          !assigned && setShowPicker({ day: dayIndex, meal })
                        }
                        onMouseEnter={(e) => {
                          if (!assigned)
                      ...
                      >
                        {assigned ? (
                          <>
                            <a
                              href={`/recipes/${assigned.recipeId}`}
                              style={{
                                fontSize: "0.75rem",
                            ...
                            >
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setShowPicker({ day: dayIndex, meal });
                                }}
                                style={{
                                  fontSize: "0.65rem",
                                  color: "var(--accent)",
              ...
              >
                ...
                >
                  <h3 style={{ fontSize: "1rem", fontWeight: 600 }}>
                    {MEAL_ICONS[showPicker.meal]} {DAYS[showPicker.day]}{" "}
                    {showPicker.meal}
                  </h3>
                  <button
                    onClick={() => setShowPicker(null)}
